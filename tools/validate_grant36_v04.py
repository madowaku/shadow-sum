"""Exhaustive independent validator for SHADOW SUM GRANT36 v0.4."""
import itertools
import json
from pathlib import Path

DATA = Path(__file__).resolve().parents[1] / "data/grant36_v0_4.json"
...
def validate():
    ...
    assert [s["id"] for s in stages] == [f"GR{i:02}" for i in range(1, 37)]

    for stage in stages:
        observation = stage["observations"][0]
        wanted = target(observation["target"])
        hidden = {cell(name) for name in observation.get("hidden_cells", [])}
        visible = [index for index in range(25) if index not in hidden]
        solutions = []
        states = 0
        ...
        for objects in object_states(stage):
            for lights in light_states(stage):
                for shutter in shutter_states(stage):
                    states += 1
                    actual = shadow(objects, lights, shutter)
                    if all(actual[index] == wanted[index] for index in visible):
