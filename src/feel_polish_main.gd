...
# 2. the full instrument should remain comfortably usable at 360x800 and 720x900.

const COMPACT_BREAKPOINT_W := 560.0
const COMPACT_SCREEN_CELL := 32.0
const COMPACT_POST_CELL := 48.0
const DESKTOP_SCREEN_CELL := 42.0
const DESKTOP_POST_CELL := 50.0
...
func _apply_responsive_layout() -> void:
	...
	if should_compact:
		...
		_set_margins(margin, 24, 24, 14, 10)
	...
	footer.add_theme_constant_override("separation", 8 if should_compact else 10)
	for child in footer.get_children():
		if child is Button:
			var button := child as Button
			button.custom_minimum_size = Vector2(104, 34) if should_compact else Vector2(112, 36)
...
func _apply_screen_layout(screens: HBoxContainer, compact: bool) -> void:
	screens.add_theme_constant_override("separation", 8 if compact else 18)
	var size_value := COMPACT_SCREEN_CELL if compact else DESKTOP_SCREEN_CELL
	var cell_font := 14 if compact else 17
	...
	for index in screens.get_child_count():
		...
		if column == null or column.get_child_count() < 2:
			continue
		column.add_theme_constant_override("separation", 3 if compact else 4)
		var screen_title := column.get_child(0) as Label
		screen_title.add_theme_font_size_override("font_size", 10 if compact else 11)
		if compact:
		...
		_set_margins(frame_margin, 4 if compact else 6, 4 if compact else 6, 4 if compact else 6, 4 if compact else 6)
...
func _apply_instrument_layout(instrument: VBoxContainer, compact: bool) -> void:
	instrument.add_theme_constant_override("separation", 3 if compact else 4)
	...
	north.custom_minimum_size = Vector2(50, 28) if compact else Vector2(60, 34)
	north.add_theme_font_size_override("font_size", 10 if compact else 11)
	arrow.add_theme_font_size_override("font_size", 14 if compact else 16)
	caption.add_theme_font_size_override("font_size", 9 if compact else 10)
	table_row.add_theme_constant_override("separation", 4 if compact else 10)
	...
	if table_row.get_child_count() >= 3:
		...
		for emitter in [west, east]:
			emitter.custom_minimum_size = Vector2(34, 34) if compact else Vector2(56, 38)
			emitter.add_theme_font_size_override("font_size", 9 if compact else 11)

		if board_panel.get_child_count() > 0:
			var board_margin := board_panel.get_child(0) as MarginContainer
			_set_margins(board_margin, 5 if compact else 8, 5 if compact else 8, 5 if compact else 8, 5 if compact else 8)
	...
	var post_size := COMPACT_POST_CELL if compact else DESKTOP_POST_CELL
