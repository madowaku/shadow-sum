...
static func matches(shadow: Array, target: Dictionary, hidden_cells: Array = []) -> bool:
	for index: int in 25:
		var code: String = String.chr(65 + index % 5) + str(int(index / 5.0) + 1)
		if hidden_cells.has(code):
			continue
...
static func solved(stage: Dictionary, posts: Array, shutters: Array, lights: Array, post_types: Dictionary = {}) -> bool:
	...
	for observation: Dictionary in stage["observations"]:
		...
		if not matches(compute_shadow(posts, active, shutters, types), observation["target"], observation.get("hidden_cells", [])):
			return false
	return true
