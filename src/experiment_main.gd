...
func load_stage(index: int) -> void:
	...
	for code: String in stage().get("fixed_posts", []):
		...
		if stage().get("tall", false) or stage().get("fixed_tall_posts", []).has(code):
			fixed_type = "tall"
		if not _socket_enabled(fixed_index) or not Optics.valid_post_type(fixed_type):
			push_error("Invalid fixed Post in " + str(stage().get("id", "unknown stage")) + ": " + code)
			continue
...
func undo_move() -> void:
	...
	for raw_index: Variant in previous.get("posts", []):
		...
		if restored_types.has(key) and Optics.valid_post_type(str(restored_types[key])):
